...
# 1. Setup Logging - Explicitly target stdout for Docker
logging.basicConfig(
    level=logging.INFO,
...
async def verify_secret(x_callback_secret: Annotated[str | None, Header()] = None):
    ...
    if x_callback_secret != CALLBACK_SECRET:
        logger.warning(f"Unauthorized request: Secret mismatch.")
        raise HTTPException(status_code=401, detail="Invalid Callback Secret")

def process_transcription(req: TranscriptionRequest):
    temp_file = f"/tmp/{req.job_id}.mp3"
    logger.info(f"--- STARTING JOB: {req.job_id} ---")
    
    try:
        # 2. Download File
        logger.info(f"Step 1/3: Downloading audio...")
        audio_response = requests.get(req.file_url)
        audio_response.raise_for_status() # Ensure the download actually worked
        
        with open(temp_file, "wb") as f:
            f.write(audio_response.content)
        logger.info(f"Download complete. Local path: {temp_file}")

        payload_content = []

        # 3. Process Based on Provider
        if req.provider == "openai":
            logger.info("Step 2/3: Processing with OpenAI Cloud API...")
            ...
            logger.info(f"Splitting into {total_chunks} chunks for OpenAI upload.")
            ...
            logger.info("Step 2/3: Processing with local Faster-Whisper...")
            segments, info = local_model.transcribe(temp_file, beam_size=5)
            logger.info(f"Local detection: {info.language} ({info.language_probability:.2%})")
        ...
        logger.info(f"Callback successful. Response: {cb_res.status_code}")
        ...
        try:
            ...
            logger.error("Could not send failure callback to CMS.")
    finally:
        if os.path.exists(temp_file):
            os.remove(temp_file)
            logger.info(f"Cleanup: Temporary file deleted.")
        logger.info(f"--- JOB {req.job_id} FINISHED ---")
...
async def start_transcription(req: TranscriptionRequest, background_tasks: BackgroundTasks):
    logger.info(f"API received request for Job: {req.job_id}")
